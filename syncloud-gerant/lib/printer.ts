...
/**
 * Generate HTML for a Bon de Livraison, Devis or Facture
 */
function generateBLHtml(data: BLData): string {
    const isQuote = data.type === "QUOTE";
    const isInvoice = data.type === "INVOICE";
    
    const primaryColor = isQuote ? "#d97706" : isInvoice ? "#2563eb" : "#10b981";
    const badgeText = isQuote ? "DEVIS PROFORMA" : isInvoice ? "FACTURE" : "BON DE LIVRAISON";
    const remaining = data.total - data.amountPaid;

    const itemRows = data.items.map((item, i) => `
        <tr>
            <td style="text-align:center;padding:6px;border-bottom:1px solid #e2e8f0;">${i + 1}</td>
            <td style="padding:6px;border-bottom:1px solid #e2e8f0;">${item.name}</td>
            <td style="text-align:center;padding:6px;border-bottom:1px solid #e2e8f0;">${item.quantity}</td>
            <td style="text-align:right;padding:6px;border-bottom:1px solid #e2e8f0;">${item.unitPrice.toLocaleString("fr-FR")}</td>
            <td style="text-align:right;padding:6px;border-bottom:1px solid #e2e8f0;font-weight:600;">${item.total.toLocaleString("fr-FR")}</td>
        </tr>
    `).join("");

    const paymentBlock = isQuote ? `
        <div class="payment-info" style="background:#fffbeb;border:1px solid #fef3c7;">
            <strong>Information :</strong> Ce document est un devis de facturation proforma valable pour une durée de 30 jours à compter de sa date d'émission.
        </div>
    ` : `
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 3px solid ${primaryColor}; }
            .company { font-size: 16px; font-weight: 800; color: #0f172a; }
            .company-info { font-size: 10px; color: #64748b; margin-top: 2px; }
            .doc-badge { background: ${primaryColor}; color: white; padding: 6px 14px; border-radius: 8px; font-weight: 700; font-size: 12px; text-align: center; }
            ...
            thead th { background: ${primaryColor}; color: white; padding: 6px 4px; font-size: 10px; font-weight: 700; text-transform: uppercase; }
            .totals { margin-left: auto; width: 200px; }
            .total-row { display: flex; justify-content: space-between; padding: 3px 0; font-size: 11px; }
            .total-row.grand { font-size: 16px; font-weight: 900; color: #0f172a; border-top: 2px solid ${primaryColor}; padding-top: 6px; margin-top: 4px; }
    ...
    <body>
        <div class="header">
            ...
            <div>
                <div class="doc-badge">${badgeText}</div>
        ...
        <div class="signatures">
            ...
            <div class="sig-box">
                <div class="sig-line">${isQuote ? "Signature Émetteur" : "Signature Livreur"}</div>
            </div>
        </div>
...
function generateThermalReceipt(data: BLData): string {
    ...
    lines.push(sep);
    lines.push(isQuote ? "DEVIS PROFORMA" : isInvoice ? "FACTURE DE VENTE" : "BON DE LIVRAISON");
    ...
    // Payment
    if (isQuote) {
        lines.push("Ce document est un devis.");
        lines.push("Valable pour 30 jours.");
    } else {
...
export async function shareBL(data: BLData): Promise<void> {
    ...
    if (await Sharing.isAvailableAsync()) {
        const title = data.type === "QUOTE" ? `Devis ${data.receiptNumber}` : `BL ${data.receiptNumber}`;
