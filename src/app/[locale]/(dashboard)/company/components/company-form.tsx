...
export const CompanyForm: React.FC<CompanyFormProps> = ({ initialData }) => {
    ...
    const { subscriptionEndsAt } = initialData;
    const isExpired = subscriptionEndsAt ? new Date(subscriptionEndsAt) < new Date() : false;
    const daysRemaining = subscriptionEndsAt ? differenceInDays(new Date(subscriptionEndsAt), new Date()) : 0;

    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 w-full pb-10">

                {/* Subscription Status Card - Full Width at Top */}
                <Card className={`shadow-md border-none ${isExpired ? 'bg-red-50 dark:bg-red-900/30' : 'bg-emerald-50 dark:bg-emerald-900/20'}`}>
                    <CardHeader className="flex flex-row items-center gap-4 space-y-0">
                        <div className={`p-2 rounded-lg ${isExpired ? 'bg-red-200 dark:bg-red-800' : 'bg-emerald-200 dark:bg-emerald-800'}`}>
                            {isExpired ? (
                                <ShieldAlert className={`w-5 h-5 ${isExpired ? 'text-red-700 dark:text-red-300' : 'text-emerald-700 dark:text-emerald-300'}`} />
                            ) : (
                                <ShieldCheck className={`w-5 h-5 ${isExpired ? 'text-red-700 dark:text-red-300' : 'text-emerald-700 dark:text-emerald-300'}`} />
                            )}
                        </div>
                        <div>
                            <CardTitle className="text-lg font-bold">Statut de l'Abonnement</CardTitle>
                            <CardDescription>
                                {subscriptionEndsAt
                                    ? `Expire le ${format(new Date(subscriptionEndsAt), "dd/MM/yyyy")}`
                    ...
                    <CardContent>
                        {subscriptionEndsAt && (
                            <p className={`font-semibold ${isExpired ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                                {isExpired
                                    ? "Votre abonnement a expiré. Veuillez contacter le support pour le renouveler."
                                    : `${daysRemaining} jour(s) restant(s) avant la prochaine échéance.`}
                            </p>
                        )}
                        {!subscriptionEndsAt && (
                            <p className="text-muted-foreground">Version illimitée ou statut non défini.</p>
                        )}
                    </CardContent>
                </Card>
