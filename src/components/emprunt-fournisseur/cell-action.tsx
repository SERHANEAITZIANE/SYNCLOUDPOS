...
export const CellAction: React.FC<CellActionProps> = ({ data, treasuryAccounts, suppliers }) => {
    ...
    // Edit form state
    const [editSupplierId, setEditSupplierId] = useState(data.supplierId || "")
    ...
    useEffect(() => {
        if (editOpen) {
            setEditSupplierId(data.supplierId || "")
    ...
    const onEdit = async () => {
        try {
            if (suppliers && suppliers.length > 0 && !editSupplierId) {
            ...
            const result = await updatePayment(data.id, {
                ...
                referenceId: editSupplierId || undefined
            })
            if (result && 'error' in result) {
                toast.error(result.error as string)
            } else {
                    ...
                    <div className="relative z-[251] bg-background rounded-lg border shadow-lg p-6 w-full max-w-[425px] mx-4">
                        <div className="flex flex-col gap-2 mb-4">
                            ...
                            <p className="text-sm text-muted-foreground">
                                Modifiez le fournisseur, le montant, le compte, l'observation ou la date.
                            </p>
                        </div>
            ...
            <div className="flex items-center gap-1">
                {data.supplierId && (
                    <Button
                        ...
                        onClick={() => router.push(`/suppliers/${data.supplierId}/ledger`)}
