...
export async function getAnalyticsData(dateRange?: { from: Date; to: Date }) {
    try {
        ...
        // ─── Calculations ─────────────────────────────────────────────────
        const posRevenue = posOrders.reduce((acc, o) => acc + Number(o.total), 0)
        const invoiceRevenue = salesOrders.reduce((acc, o) => acc + Number(o.total), 0)
        const totalRevenue = posRevenue + invoiceRevenue
        const totalExpenses = expenses.reduce((acc, e) => acc + Number(e.amount), 0)
        const totalPurchaseCost = purchases.reduce((acc, p) => acc + Number(p.total), 0)
        ...
        const outstandingDebt = customers
            ...
            .reduce((acc, c) => acc + Math.abs(Number(c.balance)), 0)
        ...
        // ─── Recent POS Orders ────────────────────────────────────────────
        const recentOrders = posOrders.slice(0, 8).map(o => ({
            id: o.id,
        ...
        const debtors = customers
            .filter(c => Number(c.balance) < 0)
            .slice(0, 5)
            .map(c => ({ id: c.id, name: c.name, balance: Math.abs(Number(c.balance)) }))
